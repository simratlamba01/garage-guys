    ...
    <>
      ...
      <section className="relative pt-40 pb-24 overflow-hidden" style={{ background: "#0B1628" }}>
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-6 text-center">
          <p className="text-xs font-bold uppercase tracking-[.16em] mb-4" style={{ color: "#FF5C1A" }}>Get In Touch</p>
          <h1 className="text-5xl font-black text-white tracking-tight leading-tight mb-4">Book Your Free Estimate</h1>
          <p className="text-lg" style={{ color: "rgba(255,255,255,.65)" }}>Fill out the form and we&apos;ll confirm your booking within 1 hour — or call us directly. We answer 7 days a week.</p>
        </div>
      </section>
      ...
      <section className="py-20" style={{ background: "#F5F7FA" }}>
        <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-5 gap-10">
          ...
          <div className="lg:col-span-2 space-y-8">
            <div>
              <h2 className="text-2xl font-black mb-6" style={{ color: "#0B1628" }}>Get In Touch Directly</h2>
              {[
                { icon: "📞", label: "Phone (7 days a week)", value: <a href="tel:+16475550192" className="text-lg font-bold" style={{ color: "#FF5C1A" }}>647-555-0192</a>, sub: "Mon–Fri 7am–8pm · Sat–Sun 8am–6pm" },
                { icon: "✉️", label: "Email Us", value: <a href="mailto:hello@junkawaypro.ca" className="font-medium" style={{ color: "#FF5C1A" }}>hello@junkawaypro.ca</a>, sub: null },
                ...
                <div key={label} className="flex gap-4 mb-6">
                  ...
                  <div>
                    <div className="text-xs font-bold uppercase tracking-wider mb-1" style={{ color: "#8896A4" }}>{label}</div>
                    <div className="text-sm" style={{ color: "#1A2535" }}>{value}</div>
                    {sub && <div className="text-xs mt-0.5" style={{ color: "#8896A4" }}>{sub}</div>}
                  </div>
                </div>
              ))}
            </div>
            <div>
              <h3 className="text-sm font-bold uppercase tracking-wider mb-3" style={{ color: "#0B1628" }}>Areas We Serve</h3>
              <div className="flex flex-wrap gap-2">
                {areas.map((a) => (
                  <span key={a} className="text-xs font-semibold px-3 py-1.5 rounded-full text-white" style={{ background: "#0B1628" }}>📍 {a}</span>
                ))}
              </div>
            </div>
            <div className="rounded-2xl overflow-hidden shadow-md" style={{ border: "1px solid #E2E8F0" }}>
              <iframe
      ...
      <section className="py-20 bg-white">
        <div className="max-w-3xl mx-auto px-6">
          <div className="text-center mb-12">
            <p className="text-xs font-bold uppercase tracking-[.16em] mb-3" style={{ color: "#FF5C1A" }}>Common Questions</p>
            <h2 className="text-4xl font-black tracking-tight" style={{ color: "#0B1628" }}>Frequently Asked Questions</h2>
          </div>
          <div className="space-y-3">
            {faqs.map((f) => (
              <details key={f.q} className="bg-white rounded-2xl group" style={{ border: "1px solid #E2E8F0" }}>
