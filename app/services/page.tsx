    ...
    <>
      ...
      <section className="relative pt-40 pb-24 overflow-hidden" style={{ background: "#0B1628" }}>
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-6 text-center">
          <p className="text-xs font-bold uppercase tracking-[.16em] mb-4" style={{ color: "#FF5C1A" }}>What We Do</p>
          ...
          <p className="text-lg" style={{ color: "rgba(255,255,255,.65)" }}>Residential or commercial, small item or full estate — JunkAway Pro handles it all with transparent pricing and a professional crew.</p>
        </div>
      </section>
      ...
      <section className="py-24 bg-white">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-14">
            <p className="text-xs font-bold uppercase tracking-[.16em] mb-3" style={{ color: "#FF5C1A" }}>Our Services</p>
            <h2 className="text-4xl font-black tracking-tight" style={{ color: "#0B1628" }}>What Can We Remove For You?</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {serviceCards.map((s) => (
              <div key={s.title} className="rounded-2xl p-6 flex gap-5 card-hover" style={{ border: "1px solid #E2E8F0" }}>
                <div className="w-14 h-14 rounded-xl flex items-center justify-center text-2xl flex-shrink-0" style={{ background: "#FF5C1A" }}>{s.icon}</div>
                <div>
                  <h3 className="text-base font-bold mb-2" style={{ color: "#0B1628" }}>{s.title}</h3>
                  <ul className="space-y-1">
                    {s.items.map((item) => (
                      <li key={item} className="text-sm flex items-center gap-2" style={{ color: "#4A5568" }}>
                        <span style={{ color: "#FF5C1A", fontWeight: 700 }}>→</span>{item}
                      </li>
                    ))}
                  </ul>
                </div>
      ...
      <section className="py-10" style={{ background: "#F5F7FA" }}>
        <div className="max-w-3xl mx-auto px-6">
          <div className="bg-white rounded-2xl p-8 shadow-sm" style={{ border: "1px solid #E2E8F0" }}>
            <h3 className="text-lg font-bold mb-2" style={{ color: "#0B1628" }}>📋 What We Can&apos;t Take</h3>
            <p className="text-sm mb-5" style={{ color: "#4A5568" }}>For safety and regulatory reasons, we&apos;re unable to haul:</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {cant.map((c) => (
                <div key={c} className="flex items-center gap-2 text-sm" style={{ color: "#4A5568" }}>
                  <span className="font-black" style={{ color: "#EF4444" }}>✕</span>{c}
                </div>
              ))}
            </div>
            <p className="text-xs mt-5" style={{ color: "#8896A4" }}>Not sure if your item qualifies? <Link href="/contact" className="font-semibold" style={{ color: "#FF5C1A" }}>Call us</Link> and we&apos;ll point you in the right direction.</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-white">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-14">
            <p className="text-xs font-bold uppercase tracking-[.16em] mb-3" style={{ color: "#FF5C1A" }}>Transparent Pricing</p>
            <h2 className="text-4xl font-black tracking-tight" style={{ color: "#0B1628" }}>Simple Volume-Based Pricing</h2>
            <p className="mt-3 max-w-md mx-auto" style={{ color: "#4A5568" }}>Price by how much space your junk takes in our truck — no hourly rates, no surprises.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {pricing.map((p) => (
              <div key={p.tier} className="rounded-2xl p-7 relative card-hover" style={{
                ...
                {p.popular && (
                  <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full text-xs font-bold text-white" style={{ background: "#FF5C1A" }}>Most Popular</div>
                )}
                <p className="text-xs font-bold uppercase tracking-wider mb-2" style={{ color: p.popular ? "rgba(255,255,255,.55)" : "#8896A4" }}>{p.tier}</p>
                ...
                <p className="text-sm mb-5" style={{ color: p.popular ? "rgba(255,255,255,.55)" : "#8896A4" }}>{p.cap}</p>
                <ul className="space-y-2 mb-7">
                  {p.items.map((item) => (
                    <li key={item} className="flex items-center gap-2 text-sm" style={{ color: p.popular ? "rgba(255,255,255,.8)" : "#4A5568" }}>
                      <span className="font-bold" style={{ color: "#FF5C1A" }}>✓</span>{item}
                    </li>
                  ))}
                </ul>
                <Link href="/contact" className="block text-center py-3 rounded-xl font-bold text-white text-sm transition-all hover:opacity-90" style={{ background: "#FF5C1A" }}>
                  Book {p.tier}
                </Link>
              </div>
            ))}
          </div>
          <p className="text-center text-xs mt-5" style={{ color: "#8896A4" }}>All prices include labour, disposal fees, and eco-recycling. HST extra. Free on-site quote before any work begins.</p>
          <div className="text-center mt-6">
            <Link href="/pricing" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-white transition-all hover:opacity-90" style={{ background: "#0B1628" }}>
