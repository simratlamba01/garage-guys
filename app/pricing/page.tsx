    ...
    <>
      ...
      <section className="relative pt-40 pb-24 overflow-hidden" style={{ background: "var(--navy)" }}>
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-6 text-center">
          <p className="text-xs font-bold uppercase tracking-[.16em] mb-4" style={{ color: "var(--orange)" }}>Live Pricing</p>
          <h1 className="text-5xl font-black text-white tracking-tight leading-tight mb-4">Pick Your Date.<br />See Your Price.</h1>
          <p className="text-lg" style={{ color: "rgba(242,223,167,.65)" }}>Click any date on the calendar to instantly see transparent, volume-based pricing for every service tier. No surprises.</p>
        </div>
      </section>

      {/* HOW PRICING WORKS */}
      <div style={{ background: "var(--navy-m)" }}>
      ...
      <section className="py-20" style={{ background: "var(--slate)" }}>
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-12">
            <p className="text-xs font-bold uppercase tracking-[.16em] mb-3" style={{ color: "var(--orange)" }}>Interactive Pricing Calendar</p>
            <h2 className="text-4xl font-black tracking-tight mb-3" style={{ color: "var(--navy)" }}>Select a Date to See Pricing</h2>
            <p className="max-w-lg mx-auto text-sm" style={{ color: "var(--text-body)" }}>
              Dates with an <span className="font-bold" style={{ color: "var(--orange)" }}>orange dot</span> have special promotional pricing.
              Dates with a <span className="font-bold" style={{ color: "var(--mid)" }}>grey dot</span> carry a weekend premium.
      ...
      <section className="py-20 bg-white">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-12">
            <p className="text-xs font-bold uppercase tracking-[.16em] mb-3" style={{ color: "var(--orange)" }}>Service Tiers</p>
            <h2 className="text-4xl font-black tracking-tight" style={{ color: "var(--navy)" }}>What Each Tier Covers</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            {[
              ...
              <div key={t.tier} className="rounded-2xl p-5 text-center card-hover" style={{ background: "#F5F7FA", border: "1px solid var(--border)" }}>
                <div className="text-3xl mb-3">{t.icon}</div>
                <div className="font-black text-base mb-1" style={{ color: "var(--navy)" }}>{t.tier}</div>
                <div className="text-xs font-bold mb-3 px-2 py-0.5 rounded-full inline-block" style={{ background: "rgba(255,92,26,.1)", color: "var(--orange)" }}>{t.cap}</div>
                <p className="text-xs leading-relaxed" style={{ color: "var(--text-body)" }}>{t.ex}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16" style={{ background: "var(--navy)" }}>
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          <p className="mb-10" style={{ color: "rgba(242,223,167,.65)" }}>Three factors determine your final price — all visible before you commit.</p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {[
              ...
              <div key={f.title} className="rounded-2xl p-7" style={{ background: "rgba(242,223,167,.06)", border: "1px solid rgba(255,255,255,.1)" }}>
      ...
      <section className="py-20" style={{ background: "var(--slate)" }}>
        <div className="max-w-3xl mx-auto px-6">
          <div className="text-center mb-12">
            <p className="text-xs font-bold uppercase tracking-[.16em] mb-3" style={{ color: "var(--orange)" }}>Questions</p>
            <h2 className="text-4xl font-black tracking-tight" style={{ color: "var(--navy)" }}>Pricing FAQ</h2>
          </div>
          <div className="space-y-3">
            {faqs.map((f) => (
              <details key={f.q} className="bg-white rounded-2xl overflow-hidden group" style={{ border: "1px solid var(--border)" }}>
